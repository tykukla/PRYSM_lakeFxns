    ...
    !f2py real(8) intent(out) :: data_output_profile, data_output_surface
      ...
      !implicit none
      include 'lake_setup.inc' ! info for simulation
...
!      rh_in = rh_in/100.
!      ea = rh_in*es
      if (dp_in.ge.1) then  ! humidity input is RH and needs to be converted to Q
         rh_in = dp_in
         rh_in = rh_in/100.
         ea = rh_in*es
         qa_in = ea*0.622/(ps_in-ea*0.378)
...
!      output files
      open(unit=50,file=output_prof,status='unknown')
      open(unit=51,file=output_surf,status='unknown')
